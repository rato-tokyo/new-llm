...
単純べき乗則よりも指数減衰モデルの方がデータに良くフィットする（AIC=16.15）。

### PPL_min（理論的下限）

| context_dim | PPL_min | 現在PPL (1600samples) |
|-------------|---------|----------------------|
| 256 | 132.5 | 134.7 |
| 320 | 122.8 | 132.6 |

**重要な発見**:
- dim=256はほぼ理論限界に到達（差2.2）
- context_dim増加の効果は限定的（25%増で7-9%改善）
- データ量増加の効果も限定的（理論限界に近いため）
