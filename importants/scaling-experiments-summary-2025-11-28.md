...
⚠️ **注意**: この実験は `max_length=128` でトークンを切り詰めています（約80 tokens/sample）
...
✅ この実験は `truncation=False` で全長を使用（約174 tokens/sample）

| 実験 | Layers | context_dim | num_input_tokens | 等差減少 | トークン化 |
|------|--------|-------------|------------------|----------|-----------|
| A | 6 | 768 | **2** | なし | 全長 |
| B | 3 | 1536 | **2** | なし | 全長 |
...
### 0. ⚠️ 実験間のトークン化設定の違い（重要）

**11/27と11/28の実験は異なるトークン化設定を使用しているため、直接比較できない**

| 実験 | トークン化設定 | tokens/sample | 500サンプル時 |
|------|---------------|---------------|---------------|
| 11/27 | `max_length=128` (切り詰め) | ~80 | 39,814 tokens |
| 11/28 | `truncation=False` (全長) | ~174 | 86,925 tokens |

**影響**:
- 同じ500サンプルでも、トークン数が**2.18倍**異なる
- α値の違い（-0.75 vs -0.29）は、この設定差による可能性が高い
- 11/27のα=-0.75を11/28のデータに適用すると、PPLを1.1〜1.6倍過小評価する
...
| データ規模 | num_input_tokens=1 α | トークン化 | 備考 |
|------------|---------------------|-----------|------|
| 小規模（〜40k tok） | **-0.72 〜 -0.75** | 切り詰め | 11/27実験 |
| 中規模（〜180k tok） | **-0.29** | 全長 | 11/28 v2実験 |

**考察**:
- α値の差はトークン化設定の違いが主因の可能性
- 統一設定での再実験なしに結論は出せない
