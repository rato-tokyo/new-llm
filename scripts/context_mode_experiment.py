#!/usr/bin/env python3
"""
Context Mode比較実験スクリプト

A案（final context only）を実験:
- 全TokenBlockレイヤーがContextBlockの最終出力のみを参照

E案（layerwise）は既に実験済みのため、本スクリプトではA案のみ実験します。
結果はE案の既存結果と比較してください。

使用方法:
  # Colab（GPU）: 2000サンプルで実験
  python3 scripts/context_mode_experiment.py

  # カスタムサンプル数
  python3 scripts/context_mode_experiment.py --samples 1000

  # context_dimを指定
  python3 scripts/context_mode_experiment.py --context-dim 768

  # 複数レイヤーを指定
  python3 scripts/context_mode_experiment.py --layers 1 2
"""
...
def run_single_experiment(
    ...
    try:
        ...
        # モデル作成（A案: use_final_context_only=True）
        set_seed(seed)
        model = LLM(
            ...
            use_final_context_only=exp_config.use_final_context_only,
        )
        model.to(device)
        ...
        print_flush(f"    Model: {total_params:,} params (layers={exp_config.num_layers}, final_context_only={exp_config.use_final_context_only})")
        ...
        # A案の場合は最終contextのみ取得（return_all_layers=Falseでも可だが、E案との比較のためTrueのままでOK）
        # ただし、A案用にキャッシュを最終レイヤーのみにする
        if exp_config.use_final_context_only:
            # A案: 全レイヤーの出力は取得するが、Phase 2では最終出力のみ使用
            train_result = phase1_trainer.train(
                train_token_ids,
                label="OACD",
                return_all_layers=True,  # メトリクス計算のため全レイヤー取得
        ...
        if exp_config.use_final_context_only:
            ...
            val_context_cache = val_context_cache_full
...
def main():
    parser = argparse.ArgumentParser(
        description='Context mode comparison experiment (A案: final context only)'
    )
    parser.add_argument(
        '--samples', '-s',
        type=int,
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=[2],
        help='Number of layers to test (default: 2)'
    )
    parser.add_argument(
        '--output-dir', '-o',
        default=None,
    ...
    if args.output_dir:
        ...
        output_dir = f"importants/logs/{timestamp}_context_mode"
    ...
    for num_layers in args.layers:
        experiment_configs.append(
            ExperimentConfig(
                name=f"L{num_layers}_A",
                num_layers=num_layers,
                use_final_context_only=True,
                description=f"layer={num_layers}, A案 (final context only)"
            )
        )

    # 情報表示
    print_flush("=" * 80)
    print_flush("CONTEXT MODE EXPERIMENT (A案: Final Context Only)")
    print_flush("=" * 80)
    ...
    print_flush(f"\n{'Config':<10} {'Layers':<8} {'Mode':<12} {'Params':<12} "
                f"{'Val PPL':<10} {'Acc':<8} {'ER%':<8} {'Time':<10}")
    print_flush("-" * 90)

    for r in results:
        mode = "A案" if r['use_final_context_only'] else "E案"
    ...
    with open(result_file, 'w') as f:
        f.write("Context Mode Experiment Results (A案: Final Context Only)\n")
        f.write("=" * 60 + "\n\n")
        ...
        for r in results:
            mode = "A案 (final_only)" if r['use_final_context_only'] else "E案 (layerwise)"
        ...
        f.write("\n\n" + "=" * 60 + "\n")
        f.write("Compare with E案 results from previous experiments:\n")
