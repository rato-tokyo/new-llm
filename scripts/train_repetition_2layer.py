#!/usr/bin/env python3
"""Repetition Training for 2-Layer New-LLM

This script trains the 2-layer variant of New-LLM to reach stable context vector fixed points.

Key Difference from 1-layer:
    - Uses NewLLM2Layer model (2 FNN layers)
    - Same training strategy as 1-layer
    - Allows comparison of context vector behavior between 1-layer and 2-layer
"""
...
def train_stage(
    ...
    print(f"\n{'='*80}")
    print(f"🎯 Stage {stage} Training (2-Layer): {loss_fn.cycle_length}-token repetition")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description="Repetition training for 2-layer New-LLM")

    # Training parameters
    parser.add_argument('--max-stage', type=int, default=1,
    ...
    print("\n🏗️  Building New-LLM 2-Layer model...")
    config = NewLLMConfig()
    ...
    model = NewLLM2Layer(config).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"✓ 2-Layer Model created: {total_params:,} parameters")
    ...
    # ========== Staged Training ==========
    print(f"\n{'='*80}")
    print("🚀 Starting Staged Repetition Training (2-Layer)")
    print(f"{'='*80}")
    ...
    for stage in range(1, args.max_stage + 1):
        ...
        checkpoint_path = os.path.join(
            args.output_dir,
            f"new_llm_2layer_repetition_stage{stage}.pt"
        )
        torch.save({
    ...
    # ========== Final Summary ==========
    print(f"\n{'='*80}")
    print("🎉 Staged Training Complete! (2-Layer)")
    print(f"{'='*80}")
    ...
    # Save final model
    final_path = os.path.join(args.output_dir, "new_llm_2layer_repetition_final.pt")
    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'config': config,
    ...
    print(f"\n✓ Final 2-layer model saved: {final_path}")
