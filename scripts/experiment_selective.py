...
- 確信度が低いときはスキップし、内部で推論を継続
- 確信度が高いときのみトークン出力

実験:
1. SelectiveOutputLM を訓練（ゲート損失付き）
2. 異なるスキップ閾値での評価
3. Pythiaベースラインとの比較
...
Usage:
    ...
    # Custom settings
    python3 scripts/experiment_selective.py --max-skip 5 --threshold 0.6
"""
...
def train_selective(
    ...
    損失 = LM損失 + gate_loss_weight * ゲート損失
    """
    best_val_ppl = float("inf")
    best_epoch = 0
    best_state_dict = None
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in train_loader:
            ...
            # Gate Loss
            gate_loss = model.compute_gate_loss(logits, gate_probs, labels)

            # Total loss
            loss = lm_loss + gate_loss_weight * gate_loss * labels.numel()
            ...
            total_gate_loss += gate_loss.item() * labels.numel()
            total_tokens += labels.numel()

        train_ppl = torch.exp(torch.tensor(total_lm_loss / total_tokens)).item()
        avg_gate_loss = total_gate_loss / total_tokens
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                logits, _, _ = model(input_ids, use_selective=False)
                loss = F.cross_entropy(
                    logits.view(-1, logits.size(-1)),
                    labels.view(-1),
                    reduction="sum",
        ...
        print_flush(
            f"  Epoch {epoch:2d}: train={train_ppl:7.1f}, val={val_ppl:7.1f}, "
            f"gate={avg_gate_loss:.4f} ({elapsed:.1f}s) {marker}"
        )
...
def main():
    ...
    print_flush(f"Evaluation thresholds: {args.thresholds}")
    print_flush("=" * 70)
    ...
    if "selective" in args.models:
        print_flush("\n" + "=" * 70)
        print_flush("SELECTIVE OUTPUT LM")
        print_flush("=" * 70)
        ...
        best_ppl, best_epoch, best_state = train_selective(
            model, train_loader, val_loader, optimizer, device,
            args.epochs, args.patience, args.gate_loss_weight, GRADIENT_CLIP,
        )
        print_flush(f"  Best: epoch {best_epoch}, ppl={best_ppl:.1f}")
        ...
        print_flush("\n  Selective Generation (different thresholds):")
    ...
    if "selective" in results:
        print_flush("\n| Threshold | Skip Ratio | Total Steps |")
        print_flush("|-----------|------------|-------------|")
        for threshold, stats in results["selective"]["generation_stats"].items():
            print_flush(
                f"| {threshold:.1f} | {stats['skip_ratio']:.1%} | {stats['total_steps']} |"
            )
