...
class ResidualConfig:
    ...
    ddr_threshold_ratio = 0.5          # 各次元の平均活性（EMA）の閾値比率
                                       # 全次元の平均活性 × threshold_ratio = 閾値
                                       # 0.5 = 平均活性の50%未満の次元のみブースト（推奨、極端に低活性な次元のみ）
                                       # 1.0 = 平均活性未満の次元すべてブースト（広範囲）
                                       # 注: 直前の出力値ではなく、EMAで追跡された長期的な活性レベルを評価
