...
| Method | PPL | KV比 | 学習時間 |
|--------|-----|------|----------|
| KV Cache (baseline) | 105.3 | - | 43s/epoch |
| KA Training | 304.0+ | +189% | 560s/epoch (13x遅い) |

**結論**:
- 3 epoch時点で val_ppl=304.0、収束の兆候なし
- **情報ボトルネック**: A[0]=V[0]の64次元が全シーケンスの唯一の情報源
- 学習速度が13倍遅い（Pythonループが必要）
- **仮説「現在位置のVは不要」は誤り**
...
当初の仮説「Residual Connectionがあるため現在位置のVは不要」は**誤り**だった。

**失敗の根本原因: 情報ボトルネック**

```
A[0] = V[0]  (64次元)
A[1] = weights @ A[0]  (A[0]の線形変換)
A[2] = weights @ [A[0], A[1]]  (加重平均)
...
A[127] = weights @ [A[0], ..., A[126]]
```

全てのA[i]は最終的にV[0]のみから派生する。新しい位置のV（新しいコンテキスト情報）が一切取り込まれないため、表現力が極端に制限される。

### 全案の失敗理由

| 案 | 失敗理由 |
|----|----------|
| 案1 (Adapter) | Train-inference mismatch |
| 案2 (KA Training) | 情報ボトルネック（Vなしでは新情報が入らない） |
| 案3 (推論のみ) | Train-inference mismatch |

### 学んだこと

1. **Vは必須**: 各位置で新しいコンテキスト情報を取り込むことがAttentionの本質
2. **Residual Connectionの誤解**: Residualは累積的な情報追加であり、Vの代替にはならない
3. **KAキャッシュの限界**: VをAで置換するアプローチは根本的に機能しない
4. **KVキャッシュ削減効果なし**: K+AとK+Vでサイズは同じ（意味がない）
...
| **案2 (KA Training)** | ❌ | 情報ボトルネックで+189%悪化、学習13倍遅い |
| **案3 (推論のみ)** | ❌ | +1037%悪化。使用不可 |

**KAキャッシュ方式は全ての実装で失敗。VをAで置換するアプローチは根本的に機能しない。**

KVキャッシュ削減が目的なら、KA方式ではなく**MLA方式**を推奨。
MLAはK+Vを低次元潜在ベクトルに圧縮し、数学的に等価な計算を行うため、精度低下なしにキャッシュを87.5%削減可能。
