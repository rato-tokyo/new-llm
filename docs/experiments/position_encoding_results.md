...
## rope_base=5000 実験（rotary_pct=1.0、パススルーなし）- 2025-12-05
...
| 回転速度 | 標準 | 約2倍 | ↑ |
| PPL | 105.6 | 105.4 | **-0.2%** |
| Overall Q max | 8.58 | 8.79 | +2% |
| Overall K max | 9.61 | 9.30 | **-3%** |
| Backward PPL | 760.1 | 681.7 | **-10.3%** ↓ |

### レイヤー別Q, K最大値の詳細比較

| Layer | Q max (base=10000) | Q max (base=5000) | 変化 | K max (base=10000) | K max (base=5000) | 変化 |
|-------|-------------------|-------------------|------|-------------------|-------------------|------|
| 0 | 7.96 | 6.18 | **-22%** | 7.33 | 6.72 | -8% |
| 1 | 7.41 | 6.47 | **-13%** | 9.61 | 9.30 | -3% |
| 2 | 7.51 | 8.05 | +7% | 7.24 | 8.10 | +12% |
| 3 | 7.80 | 8.63 | +11% | 9.39 | 8.87 | -6% |
| 4 | 8.58 | 8.79 | +2% | 8.42 | 8.02 | -5% |
| 5 | 8.23 | 7.82 | -5% | 7.89 | 6.99 | **-11%** |

**傾向**:
- **前半レイヤー（0-1）**: Q, K両方で値が減少（-8%〜-22%）
- **中間レイヤー（2-3）**: Qは増加、Kは変動
- **後半レイヤー（4-5）**: K maxが減少傾向

### 周波数帯域別の詳細比較

| 領域 | Q max (base=10000) | Q max (base=5000) | 変化 | K max (base=10000) | K max (base=5000) | 変化 |
|------|-------------------|-------------------|------|-------------------|-------------------|------|
| 高周波 (0-31) | 8.46 | 8.79 | +4% | 9.61 | 9.26 | -4% |
| 低周波 (32-63) | 8.58 | 8.53 | -1% | 9.33 | 9.30 | 0% |
| low/high ratio (Q) | 1.01x | 0.97x | -4% | - | - | - |
| low/high ratio (K) | 0.97x | 1.00x | +3% | - | - | - |

### Position-wise PPL 比較

| Position | base=10000 | base=5000 | 変化 |
|----------|------------|-----------|------|
| 0-16 | 153.9 | 143.7 | **-6.6%** ↓ |
| 16-32 | 107.3 | 105.6 | -1.6% |
| 32-64 | 107.2 | 104.6 | -2.4% |
| 64-96 | 104.4 | 101.7 | -2.6% |
| 96-128 | 105.2 | 102.5 | -2.6% |

**観察**:
- **序盤位置（0-16）で顕著な改善（-6.6%）**
- 全position範囲で改善傾向
...
**2. Reversal Curseが大幅改善**
- Backward PPL: 760.1 → 681.7（**-10.3%**）
- 高速回転により**双方向の情報伝達が改善**した可能性
- これはrotary_pct=0.25の結果（+1.7%悪化）と**逆の傾向**

**3. 前半レイヤーでQ, K値が減少**
- Layer 0: Q -22%, K -8%
- Layer 1: Q -13%, K -3%
- 高速回転により初期レイヤーの活性化が抑制される

**4. 序盤位置のPPLが改善**
- Position 0-16: 153.9 → 143.7（**-6.6%**）
- 高速回転により近距離の位置情報がより明確に

**5. low/high ratioは依然として≈1.0**
- Q: 1.01x → 0.97x（高周波が相対的に増加）
- K: 0.97x → 1.00x（ほぼ同等）
- **Massive Values仮説は再現されず**

### rotary_pct=0.25 vs rotary_pct=1.0 での回転速度効果の比較

| 指標 | rotary_pct=0.25 | rotary_pct=1.0 |
|------|-----------------|----------------|
| PPL変化 | +0.5% (悪化) | **-0.2% (改善)** |
| Reversal Curse | +1.7% (悪化) | **-10.3% (改善)** |
| K max変化 | +10% (増加) | **-3% (減少)** |
| 序盤PPL (0-16) | +0.7% (悪化) | **-6.6% (改善)** |

**重要な発見**: 回転速度を2倍にした効果は`rotary_pct`によって**逆転**する：
- rotary_pct=0.25: 全体的にわずかに悪化
- rotary_pct=1.0: 全体的に改善、特にReversal Curseが大幅改善
...
|------|------------|-----------|------|
| PPL | 105.6 | 105.4 | -0.2% |
| Reversal Curse | 760.1 | 681.7 | **-10.3%** |
| 序盤PPL (0-16) | 153.9 | 143.7 | **-6.6%** |
| low/high ratio | ≈1.0 | ≈1.0 | 同等 |

**rotary_pct=1.0での回転速度2倍の効果**:
- PPLへの影響は微小だが改善方向（-0.2%）
- **Reversal Curseが大幅改善（-10.3%）**
- **序盤位置のPPLが改善（-6.6%）**
- 全次元にRoPEを適用した状態では、高速回転が有利
...
Last Updated: 2025-12-05 (rotary_pct=1.0 + rope_base=5000 純粋回転速度比較実験追加)
