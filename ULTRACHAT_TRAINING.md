# UltraChat Training Guide

大規模多様対話データセット（UltraChat）での訓練ガイド

---

## 📊 UltraChatとは

**GPT-3.5生成の超大規模対話データセット**

| 特徴 | 詳細 |
|-----|------|
| **データ量** | 1,500,000件以上（HH-RLHFの**18倍**） |
| **形式** | 複数ターン対話（Multi-turn） |
| **言語** | 英語のみ |
| **トピック** | 科学、技術、歴史、文化、哲学など**非常に多様** |
| **生成方法** | GPT-3.5で生成（高い多様性） |

---

## 🎯 HH-RLHFとの比較

| 項目 | HH-RLHF | UltraChat |
|-----|---------|-----------|
| **データ量** | 85,000 | **1,500,000（18倍）** |
| **トピック多様性** | 標準 | **非常に高い** |
| **訓練時間** | 20-30分 | **2-3時間（全データ）** |
| **期待PPL** | 17-20 | **20-25** |
| **難易度** | ★★☆☆☆ | **★★★☆☆** |

**UltraChatの方が難しい**が、より多様な対話能力を獲得できます。

---

## 🚀 Google Colabでの実行方法

### ⚠️ 重要：データセット規模について

UltraChatは**1.5M件**と非常に大規模です。以下の選択肢があります：

1. **フルデータセット（1.5M件）**: 2-3時間の訓練時間
2. **サブセット（10万件など）**: 20-40分の訓練時間（推奨）

### ステップ1: 環境セットアップ

```python
# GPU確認
!nvidia-smi

# リポジトリクローン
%cd /content
!rm -rf new-llm
!git clone https://github.com/rato-tokyo/new-llm
%cd new-llm

# 依存関係インストール
!pip install -q datasets
```

### ステップ2: UltraChat訓練開始

#### オプションA: サブセット訓練（推奨、20-40分）

```bash
# Layer 1（10万件サンプル）
!nohup python3 scripts/train_ultrachat.py --num_layers 1 --max_samples 100000 > /content/ultrachat_layer1_100k.log 2>&1 &

# ログ確認
!tail -20 /content/ultrachat_layer1_100k.log

# GPU使用状況
!nvidia-smi
```

#### オプションB: フルデータセット訓練（2-3時間）

```bash
# Layer 1（全1.5M件）
!nohup python3 scripts/train_ultrachat.py --num_layers 1 > /content/ultrachat_layer1_full.log 2>&1 &

# ログ確認
!tail -20 /content/ultrachat_layer1_full.log
```

### ステップ3: Layer 4で訓練（推奨）

```bash
# Layer 4（10万件サンプル）
!nohup python3 scripts/train_ultrachat.py --num_layers 4 --max_samples 100000 > /content/ultrachat_layer4_100k.log 2>&1 &

# ログ確認
!tail -20 /content/ultrachat_layer4_100k.log
```

---

## 📈 期待される結果

### サブセット（10万件）

| 指標 | Layer 1 | Layer 4 |
|-----|---------|---------|
| **Val PPL** | **20-23** | **18-21** |
| **Val Acc** | **41-43%** | **43-45%** |
| **訓練時間** | 20-30分 | 30-40分 |

### フルデータセット（1.5M件）

| 指標 | Layer 1 | Layer 4 |
|-----|---------|---------|
| **Val PPL** | **18-21** | **16-19** |
| **Val Acc** | **43-45%** | **45-47%** |
| **訓練時間** | 2-3時間 | 3-4時間 |

**HH-RLHFとの比較**:
- HH-RLHF Layer 1: PPL 17-20
- UltraChat Layer 1: PPL 20-23（+3ポイント難しい）
- UltraChat Full Layer 4: PPL 16-19（最高性能）

---

## ⚙️ 設定

### 自動設定（デフォルト）

| パラメータ | 値 | 理由 |
|-----------|-----|------|
| **batch_size** | 2048 | L4 GPU最適化 |
| **learning_rate** | 0.0008 | Square Root Scaling |
| **max_seq_length** | 128 | 複数ターン対話用 |
| **epochs** | 50 | 大規模データセット用（少なめ） |
| **context_dim** | 256 | 標準 |

### カスタマイズ

```bash
# より長いシーケンス
python scripts/train_ultrachat.py --num_layers 4 --max_seq_length 256

# エポック数調整
python scripts/train_ultrachat.py --num_layers 4 --max_samples 100000 --epochs 100

# 小規模テスト（1万件）
python scripts/train_ultrachat.py --num_layers 1 --max_samples 10000
```

---

## 🔍 進捗モニタリング

### リアルタイムログ表示

```bash
# 最新20行
!tail -20 /content/ultrachat_layer1_100k.log

# 継続的な監視
!tail -f /content/ultrachat_layer1_100k.log
```

### ✓マークの意味

```
Epoch 25/50
  Training... 100% | 1.2min | Loss: 2.95
  Val: Loss=2.91 PPL=18.4 Acc=44.2% ✓
  [Checkpoint saved]
  ↑
  ✓ = このEpochがベストモデル
```

### GPU使用状況

```bash
!nvidia-smi
```

---

## 📊 結果の解釈

### 成功の指標

| PPL範囲 | 評価 | 次のステップ |
|---------|------|-------------|
| **< 19** | 🏆 **優秀** | Level 4（CodeAlpaca）へ |\
| **19-24** | ✅ **成功** | フルデータセットで再挑戦 or Level 4へ |
| **> 24** | ⚠️ **要改善** | サンプル数を増やす or Layer増やす |

### HH-RLHFとの比較

**期待されるパターン**:
```
HH-RLHF (85k): PPL 17-20 ← 高品質だが限定的
UltraChat (100k): PPL 20-23 ← 多様性で難しい
UltraChat (1.5M): PPL 18-21 ← 大量データで改善
```

**これは正常です！** UltraChatはより多様で難しいタスクです。

---

## 🎯 何を学習するか

### HH-RLHFで学んだこと

- 高品質な複数ターン対話
- 人間が好む応答パターン
- 安全性（有害応答の回避）

### UltraChatで学ぶこと

- **超多様なトピック**: 科学、技術、歴史、文化、哲学など
- **様々な会話スタイル**: GPT-3.5が生成した多様なパターン
- **実践的な対話能力**: 様々な場面での対話対応
- **大規模データからの学習**: 1.5M件の豊富な知識

---

## ⚡ トラブルシューティング

### GPU Out of Memory

```bash
# batch_sizeを減らす
# src/utils/config.pyのNewLLML4Configを編集
batch_size = 1024  # 2048 → 1024
```

### 訓練が遅い

```bash
# サンプル数を減らす
python scripts/train_ultrachat.py --num_layers 1 --max_samples 50000

# Layer数を減らす
python scripts/train_ultrachat.py --num_layers 1  # 4 → 1
```

### PPLが下がらない

1. **サンプル数を増やす**: 10万 → 50万
2. **Epochを増やす**: 50 → 100
3. **Layer 4で試す**: より深いモデル
4. **フルデータセット**: 1.5M全件で訓練

---

## 📁 保存されるファイル

| ファイル | 説明 |
|---------|------|
| `checkpoints/best_new_llm_ultrachat_layers1.pt` | ベストモデル |
| `new_llm_ultrachat_layers1_final.pt` | 最終Epoch |
| `/content/ultrachat_layer1_100k.log` | 訓練ログ |

---

## 🚀 訓練完了後

### 結果の保存

```bash
# Colabからダウンロード
from google.colab import files
files.download('/content/new-llm/checkpoints/best_new_llm_ultrachat_layers1.pt')
files.download('/content/ultrachat_layer1_100k.log')
```

### 次のステップ

**成功したら（PPL < 24）**:

1. **フルデータセット**: 1.5M件で再訓練（さらに性能向上）
2. **Layer 4で試す**: より高性能
3. **Level 4へ進む**: CodeAlpaca（コード生成能力）
4. **Context Expansion**: 256→512次元（より長い文脈）

**対話能力の完成！**: UltraChatで成功すれば、普通に対話できるレベルに到達

---

## 📖 関連ドキュメント

- `TRAINING_PROGRESSION.md` - データセット難易度順
- `HH_RLHF_TRAINING.md` - HH-RLHF訓練ガイド
- `experiments/dolly_dialog_experiment_2025-11-19.md` - Dolly結果
- `ARCHITECTURE.md` - New-LLMアーキテクチャ

---

## 💡 推奨される訓練戦略

### ステップ1: サブセットで試す（推奨）

```bash
# まず10万件で試して、動作確認・性能評価
python scripts/train_ultrachat.py --num_layers 4 --max_samples 100000
```

**利点**:
- ✅ 20-40分で完了（Colab 90分制限内）
- ✅ 動作確認とデバッグが早い
- ✅ 性能の見通しが立つ

### ステップ2: フルデータセット（成功後）

```bash
# サブセットで良好な結果なら、フルデータセットで訓練
python scripts/train_ultrachat.py --num_layers 4
```

**利点**:
- ✅ 最高性能を獲得
- ✅ より多様な対話パターンを学習

---

**準備完了！** UltraChat訓練を開始してください。

**推奨開始コマンド**:
```bash
# サブセット（10万件）で開始
python scripts/train_ultrachat.py --num_layers 4 --max_samples 100000
```
