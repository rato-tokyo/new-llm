...
## 🧠 Context-Dependent Reasoning Training（CDR訓練）

**目的: 推論パターンをFFNに学習させ、知識は外部（コンテキスト）に預ける**

### 背景と動機

Reversal Curseの根本原因は、LLMが**知識と推論を分離できていない**こと。

```
従来のLM訓練:
  "Tom is Alice's parent." を丸暗記
  → FFNに「Tom→Alice」という具体的事実が保存される
  → 逆方向「Alice→Tom」は学習されていないため推論できない
```

**CDR訓練の仮説**:
- **推論パターン**（親→子、子→親の関係性）をFFNに学習させる
- **具体的知識**（誰が誰の親か）はAttentionでコンテキストから取得させる
- これにより、方向に依存しない推論が可能になる

### 訓練データ設計

#### Baseline（従来方式）

コンテキストと質問応答を連結して、全体を次トークン予測として学習：

```
入力: "Tom is Alice's parent. Who is Tom's child? Alice"
ラベル: 全トークン（文全体を学習）
```
...
```
初期コンテキスト: "Tom is Alice's parent."
学習対象: "Who is Tom's child?" → " Alice"
         "Who is Alice's parent?" → " Tom"
```

- コンテキスト部分は入力として与えるが、loss計算から除外
- 質問→回答の部分のみ学習
- **順方向・逆方向の両方を学習**
...
```
初期コンテキスト: ""（空）
学習対象: "Who is Tom's child?" → " I don't know"
```

- 知識がない状態では「わからない」と回答することを学習
- これにより、FFNは**事実を丸暗記しない**

### なぜこれが機能するか

1. **FFNは推論パターンを学習**
   - 「parent」「child」という関係性の抽象的パターン
   - コンテキストから情報を抽出する方法

2. **知識はAttentionで処理**
   - 具体的な名前（Tom, Alice）はコンテキストから取得
   - FFNに事実を保存しないため、方向依存性が発生しない

3. **「わからない」の学習が重要**
   - コンテキストなしで答えられないことを明示的に学習
   - FFNが事実を記憶しようとするインセンティブを除去
...
| 評価項目 | 説明 |
|----------|------|
| コンテキストあり Forward | コンテキストを与えて順方向質問 |
| コンテキストあり Backward | コンテキストを与えて逆方向質問 |
| コンテキストなし | 知識なしで質問 → "I don't know"が正解 |

**成功の指標**:
1. コンテキストありで Forward/Backward のGapが小さい
2. コンテキストなしで "I don't know" を正しく出力
3. 一般的な言語能力（Pile PPL）が維持される
