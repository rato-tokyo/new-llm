...
## 📈 スケーリング則: 飽和モデルが最適 (2025-12-03)

**⚠️ PPLのスケーリングは飽和モデル（Saturation Model）に従う。**

### スケーリングモデル比較 (5点データ: 200-3200 samples)
...
| 構成 | combined_dim | Samples | Val PPL | 備考 |
|------|--------------|---------|---------|------|
| p=0 | 512 | 1600 | 127.4 | ベースライン |
| **i=1, c=1** | 1024 | 1600 | **118.2** | **連続1つ前（推奨）** |
| i=8, c=2 | 1536 | 800 | 162.1 | 間隔8（効果なし） |

**結論**: 連続した履歴（interval=1）が効果的。大きなintervalは悪化する。

### 非均等分割の実験結果 (2025-12-03)

| 構成 | combined_dim | 800 samples PPL | Val Acc | 備考 |
|------|--------------|-----------------|---------|------|
| [256, 256] | 512 | 150.5 | 23.5% | 均等分割（ベースライン） |
| **[256, 128]** | 384 | 159.6 | 23.2% | 非均等分割 |

**分析**:
- 次元を25%削減（512→384）で、PPLは6%悪化（150.5→159.6）
- パラメータ効率向上のオプションとして有効
- **精度優先なら均等分割を推奨**
...
Last Updated: 2025-12-03 (prev_context interval=8実験: 効果なし確認)
