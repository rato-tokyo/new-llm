...
```
max_skip=1, threshold=0.5:
  Epoch 1: train=754.2, val=919.0, carry=47.3%
  Epoch 2: train=251.6, val=467.1, carry=45.1%
```
...
- 動的な判断より**固定パターン**（例: 2回に1回持ち越し）がシンプル
- 学習可能ゲートは追加パラメータ（65K）に対して効果が見合わない
- OutputGate、エントロピーベース損失は削除し、固定スキップパターンに簡素化
...
| 日付 | 内容 |
|------|------|
| 2025-12-07 | **学習可能ゲート失敗を記録**: OutputGate方式は複雑すぎて失敗、固定スキップパターンに簡素化 |
